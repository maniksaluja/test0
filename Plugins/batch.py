...
async def send_delayed_message(chat_id, text, reply_markup=None, delay=3):
    """Send message with delay to avoid rate limiting issues."""
    await asyncio.sleep(delay)  # Delay to handle rate limit
    return await sendMessage(chat_id, text, reply_markup)  # Send message using your existing sendMessage function
...
async def end(_, m):
    ...
    for x in ms:
        ...
        await send_delayed_message(m.chat.id, "Sending message... Please wait.")
