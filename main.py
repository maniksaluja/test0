...
class ClientLike(Client):
    ...
    async def track_response_time(self, message_func, *args, **kwargs):
        ...
        try:
            ...
            if args[0] is None:
                print("[ERROR] Channel ID is None. Skipping.")
            ...
            if hasattr(sent_msg, 'message_id'):
                ...
                print(f"Message deleted from channel {args[0]} with message_id {sent_msg.message_id}")
            else:
                print(f"[ERROR] Message does not have message_id. Not deleting.")
    ...
    for i, channel in enumerate(channels_to_check):
        if channel is not None:
            try:
                ...
                print(f"[ERROR] Cannot send message in channel {channel}. Error: {e}")
                ret = True
